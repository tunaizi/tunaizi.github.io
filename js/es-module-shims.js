...
;(function () {
  ...
  const skip = esmsInitOptions.skip
    ? new RegExp(esmsInitOptions.skip)
    : null
  ...
  const featureDetectionPromise = Promise.resolve(
    supportsImportMaps || supportsDynamicImportCheck
    ...
    return Promise.all([
      ...
      cssModulesEnabled &&
        dynamicImport(
          createBlob(
            `import"${createBlob('', 'text/css')}"assert{type:"css"}`
          )
        ).then(() => (supportsCssAssertions = true), noop),
      jsonModulesEnabled &&
        dynamicImport(
          createBlob(
            `import"${createBlob('{}', 'text/json')}"assert{type:"json"}`
          )
            ...
            new Promise((resolve) => {
              ...
              iframe.onload = () => {
                ...
                const supportsSrcDoc =
                  iframe.contentDocument.head.childNodes.length > 0
                const importMapTest = `<!doctype html><script type=importmap nonce="${nonce}">{"imports":{"x":"${createBlob(
                  ''
                )}"}<${''}/script><script nonce="${nonce}">import('x').catch(() => {}).then(v=>parent._$s(!!v))<${''}/script>`
                if (supportsSrcDoc) iframe.srcdoc = importMapTest
  ...
  function parse(k, l = '@') {
    ...
    if (u > s || !e) {
      ...
      ;(a = new ArrayBuffer(s)),
        ...
        (e = (function (e, a, r) {
          ...
          function Q() {
            return (
              (((t[((t[49] | 0) + 8) >> 2] | 0) - (t[3] | 0)) >> 1) | 0
            )
          }
          function R() {
            return (
              (((t[((t[50] | 0) + 4) >> 2] | 0) - (t[3] | 0)) >> 1) | 0
            )
          }
          function T(e) {
            e = e | 0
    ...
    for (; e.re(); ) {
      const a = e.es(),
        r = c$1.charCodeAt(a)
      d.push(
        34 === r || 39 === r ? b(a + 1, r) : c$1.slice(e.es(), e.ee())
      )
    }
    return [w, d, !!e.f()]
  }
  ...
  const resolveSync = (id, parentUrl = baseUrl) => {
    parentUrl = `${parentUrl}`
    const result =
      resolveHook && resolveHook(id, parentUrl, defaultResolve)
            ...
            for (const node of mutation.addedNodes) {
              if (node.tagName === 'SCRIPT') {
                if (node.type === (shimMode ? 'module-shim' : 'module'))
                  processScript(node)
                if (
                  node.type === (shimMode ? 'importmap-shim' : 'importmap')
                )
                  processImportMap(node)
              } else if (
                node.tagName === 'LINK' &&
                node.rel ===
                  (shimMode ? 'modulepreload-shim' : 'modulepreload')
              )
                processPreload(node)
            }
          }
      ...
      importMapPromise = importMapPromise
        .then(async () => {
          importMap = resolveAndComposeImportMap(
            script.src
              ? await (
                  await doFetch(script.src, getFetchOpts(script))
                ).json()
