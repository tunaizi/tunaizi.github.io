...
export function convertToExampleData(): ExampleData {
  ...
  const runXml = (): ExampleData => {
    let txt = runForRawCode().trim()
...
<template>
  <div v-html="content"></div>
    ...
    const svgTagPattern =
      /^<\s*(rect|circle|path|g|line|polyline|polygon|ellipse|text|tspan|defs|linearGradient|radialGradient|stop|clipPath|mask|pattern|symbol|use|image|foreignObject)[\s>]/i
    if (svgTagPattern.test(txt)) {
      txt = `
          <svg
